...
let BlobImpl = global.Blob;
let FormDataImpl = global.FormData;
let fetchImpl = global.fetch;
try {
  const undici = require("undici");
  BlobImpl = undici.Blob;
  FormDataImpl = undici.FormData;
...
async function run() {
  ...
  require("fs").writeFileSync("./scripts/talatona_books.xlsx", Buffer.from(buf));
  const blob = new BlobImpl([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
  const form = new FormDataImpl();
  form.append("file", blob, "talatona_books.xlsx");
  const res = await (fetchImpl || fetch)("http://localhost:3001/api/products/import", { method: "POST", body: form });
  const json = await res.json();
  require("fs").writeFileSync("./scripts/import_result.json", JSON.stringify(json, null, 2));
}

run().catch((e) => {
  require("fs").writeFileSync("./scripts/import_error.txt", String(e?.stack || e?.message || e));
  process.exit(1);
});
